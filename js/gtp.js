...
(function($){
	...
	$.fn.googleTrafficPop = function(options){
			...
			getPopHTML = function() {
				...
				if(defaults.closeable == true){
					...
					gpClose = '<a href="#" onClick="gppFlush();" id="gpp-close">Close<a/>';
					
				}
								
				var tPop = '<div id="gpp-bg"></div><div id="gpluspop"><div id="gpp-title">'+gpClose+''+defaults.title+'<div style="clear:both;"></div></div><div id="gpp-msg">'+defaults.message+'<div id="gpp-button"><g:plusone size="standard" count="true" href="'+defaults.url+'" callback="gppCB"></g:plusone></div></div><div id="gpp-bottom"><div id="gpp-counter">Or wait <span id="gpp-count"></span> seconds.</div></div></div>';
			...
			var cook = readCookie('gpluspop');
		
			// Get wait cookie
			var waitCook = readCookie('gppwait');
			...
			if(cook != 'true' && waitCook != 'true'){
				...
				if(defaults.timeout == '0'){
					...
					$('#gpp-count').countdown({until: '+'+defaults.timeout+'s', format: 'S', compact: true, description: '', onExpiry: gppFlush});
...
function gppFlush(action){
	...
	jQuery("#gpp-bg").fadeOut("slow");
			
	// Fade out the pop up itself
	jQuery("#gpluspop").fadeOut("slow");
...
jQuery(document).ready(function() {                                                                    
	jQuery().googleTrafficPop({
		'timeout':              10,
		'title':                "Welcome!",
		'message':              "Share my site with Google +1 and see what happens!",
